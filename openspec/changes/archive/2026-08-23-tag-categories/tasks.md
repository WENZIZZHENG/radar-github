...
- [x] 5.1 独立评审子 agent，中/高 findings 清零（最多三轮）
- [x] 5.2 A 级预演（真实库：建表/新建空分类/归类/一标签多分类/改名合并/删除级联＋未分类回落逐项过）＋真实页面截图＋复验步骤卡 → 本人走查
- [x] 5.3 部署生产（启动自动建表）并冒烟（/tags 分组渲染＋一轮真实 CRUD）
- [x] 5.4 通过后当场归档（任务拆解表主表行＋详情卡搬归档，openspec archive），文档按 §5 节奏提交
